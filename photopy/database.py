...
SQL_TABLE_CREATE_FILES = f"""CREATE TABLE {TABLE_NAME_FILES}(path TEXT NOT NULL PRIMARY KEY, size INTEGER, hash TEXT);"""
SQL_INSERT_RECORDS_FILES = f"""INSERT INTO {TABLE_NAME_FILES} (path, size, hash) VALUES(?,?,?);"""
SQL_SELECT_RECORDS_FILES = f"""SELECT * FROM {TABLE_NAME_FILES} ORDER BY hash SESC, size DESC"""
